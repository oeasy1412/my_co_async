...
struct SimpleMap {
    SimpleMap() = default;
    SimpleMap(std::initializer_list<std::pair<K, V>> init) : mData(init.begin(), init.end()) {}
    ...
    const V* at(const Key& key) const noexcept {
    ...
    V& insert_or_assign(K key, V value) {
        return mData.insert_or_assign(std::move(key), std::move(value)).first->second;
    }

    V& insert(K key, V value) { return mData.emplace(std::move(key), std::move(value)).first->second; }
    ...
    contains(Key&& key) const noexcept {
